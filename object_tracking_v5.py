...
# Shu's path
# test_1_txt_filename = "G:/9517GroupProject/txt_files/try_1_4/test_1.txt"    # path of the txt file
# test_1_jpg_dir = "G:/9517GroupProject/step_images/test/STEP-ICCV21-01/" # path of dir of test 1 images
# test_1_out_dir = "G:/9517GroupProject/output_images/test/new_try_1_4/" # output directory
...
def detect_and_tracks(txt_filename, jpg_dir, out_dir):
    ...
    jpg_list = video_crop.sort_file_in_correct_order(jpg_list)########################
    # now we need to draw rectangulars and lines use id_tracks
    frame = 0
    cv2.namedWindow("video")########################
    cv2.setMouseCallback("video", video_crop.crop_image)########################
    curr_ids = []
    prev_ppl = {}
    while frame < n_frames: ##
        ...
        id = 0
        while id < n_ids:
            if id_tracks[id][frame].all() != 0 and id_tracks[id][frame].all() != -1:
                ...
                status, groups = analyses.dectect_sorrounding(id_tracks, frame, id, status_list, groups)###
                #print(groups)
                # draw box and id
                ...
                #cv2.putText(img, status, (center_x, center_y + height//2), 0, 1, colours[id], 2)###
        ...
        analyses.draw_groups(groups, id_tracks, frame, img)
        
        #Detect people entering or leaving the screen
        analyses.entering_leaving_screen(prev_ppl, curr_ppl, colours, img)
        if frame > 2 and frame%7 == 0:
            prev_ppl = curr_ppl

        # add current total and number of person in this frame to the image (2.1 and 2.2)
        cv2.putText(img, "Current Total Number of Persons: " + str(curr_total_n_person), (10, 30), 0, 1, (0, 255, 255), 2)
        cv2.putText(img, "Number of Persons This Frame: " + str(n_person_this_frame), (10, 60), 0, 1, (0, 255, 255), 2)
        
        video_crop.run_video_with_crop(img, all_label[frame])########################
